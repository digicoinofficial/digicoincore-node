...
digicoin.prototype.subscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'digicoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
...
digicoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'digicoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
    ...
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('digicoind/addressbalance', {
                    address: address,
                    txid: txid,
 ...
 * @param {Array} addresses - A array of digicoin addresses to filter
 * @param {Function} callback
 */
